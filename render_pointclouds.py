...
SEQUENCES = {
    "room2": {
        ...
        "out_rgb": "outputs/room2/pointcloud_full_rgb.png",
    ...
    "corridor3": {
        ...
        "out_rgb": "outputs/corridor3/pointcloud_full_rgb.png",
    ...
    "outdoors5": {
        ...
        "out_rgb": "outputs/outdoors5/pointcloud_full_rgb.png",
...
def render(seq, cfg):
    ...
    plt.savefig(cfg["out"], dpi=120, facecolor=fig.get_facecolor(), bbox_inches="tight")
    plt.close()
    print(f"  [{seq}] saved → {cfg['out']}")

    # RGB (greyscale intensity) view — same figure size as depth view
    fig = plt.figure(figsize=(5, 4), facecolor="#0d0d0d")
    ax  = fig.add_subplot(111, projection="3d", facecolor="#0d0d0d")
    ax.scatter(
        xyz[:, 0], xyz[:, 2], -xyz[:, 1],
        c=rgb, s=0.3, alpha=0.8, linewidths=0,
    )
    _setup_ax(ax, cfg)
    plt.tight_layout(pad=0.5)
    plt.savefig(cfg["out_rgb"], dpi=120, facecolor=fig.get_facecolor(), bbox_inches="tight")
    plt.close()
    print(f"  [{seq}] saved → {cfg['out_rgb']}")
